...
class MyApp(QMainWindow):
    ...
    def set_ui(self):
        ...
        self.btn_img.setToolTip('이미지 불러오기')
        ...
        self.undo_img.setToolTip('실행 취소')
        self.undo_img.clicked.connect(self.undo_action)
        ...
        self.redo_img.setToolTip('다시 실행')
        self.redo_img.clicked.connect(self.redo_action)
        ...
        self.save_img.setToolTip('이미지 저장')
    ...
    def load_img(self):
        ...
        # 제목 설정
        self.img_title.setText(QFileInfo(open_file[0]).fileName())
    ...
    def undo_action(self):
        if self.view.undo_list:
            item = self.view.undo_list.pop()  # 리스트에서 마지막 항목 제거
            self.view.redo_list.append(item)

            if isinstance(item, list):
                for i in item:
                    self.view.scene.removeItem(i)  # 장면에서도 제거
            else:
                self.view.scene.removeItem(item)


    def redo_action(self):
        if self.view.redo_list:
            item = self.view.redo_list.pop()  # 리스트에서 마지막 항목 제거
            self.view.undo_list.append(item)

            if isinstance(item, list):
                for i in item:
                    self.view.scene.addItem(i)  # 장면에서 추기
            else:
